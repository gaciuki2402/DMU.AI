...
def update_index_with_interaction(question, answer, conversation_id):
    ...
    logger.info(f"Added new interaction to the index for conversation {conversation_id}")
...
def get_relevant_context(question, conversation_id):
    ...
    if vectorstore:
        ...
        for i, doc in enumerate(relevant_docs):
            logger.info(f"Document {i+1} content (first 200 chars): {doc.page_content[:200]}...")
        ...
        logger.warning("Vectorstore is not initialized. Returning only conversation history.")
